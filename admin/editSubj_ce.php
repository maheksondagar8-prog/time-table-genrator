...
<form action="" method="post">
	...
	$qq = "select * from subj_ce where sid='$id'";
	...
	if(isset($_POST['s1']))
	{
		...
		$qq = "UPDATE subj_ce SET sem='$t1',sname='$t2' where sid='$a'";

		$rr = mysqli_query($con,$qq);

		if($rr > 0)
		{
				echo "<script>alert('Record Edited Successfully');window.location='subj_ce.php';</script>";
		}
		else
		{
				echo "<script>alert('Error: Record Could not be edit');window.location='subj_ce.php';</script>";
		}
	}
	
	if(isset($_POST['s2']))
	{
		header("Location: subj_ce.php");
	}
	?>
